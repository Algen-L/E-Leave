...
@push('styles')
    <style>
        ...
        .step-label {
            ...
            font-weight: 600;
            display: none;
            /* Hide primarily, show on hover? */
        }

        .stepper:hover .step-label {
            display: block;
        }
    </style>
    <style>
...
@section('content')
    ...
    <div class="history-card">
        <div class="history-header">
            <h2 class="font-bold text-lg" style="margin: 0; line-height: 1;"><i class="fas fa-history mr-2"></i> Application
                History</h2>
        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr>
                        <th style="padding-left: 24px;">Date Filed</th>
                        <th>Leave Type</th>
                        <th>Inclusive Dates</th>
                        <th class="text-center">Progress Tracker</th>
                    ...
                    @forelse($applications as $app)
                        @php
                            ...
                            if (str_contains(strtolower($app->status), 'reject') || str_contains(strtolower($app->status), 'disapprove')) {
                                if (!$app->hr_verified_at)
                                    $s1 = 'rejected';
                                else if (!$app->recommended_at)
                                    $s2 = 'rejected';
                                else
                                    $s3 = 'rejected';
                            }
                        ...
                        <tr onclick="window.location='{{ route('user.leave.show', $app->id) }}'" style="cursor: pointer;">
                            ...
                            <td class="text-center" style="padding-right: 24px;">
                                <div class="flex justify-center gap-2" onclick="event.stopPropagation()">
                                    <a href="{{ route('user.leave.show', $app->id) }}" class="action-btn text-blue-600"
