...
<div class="management-tabs">
    <a href="{{ route('admin.manage-users', ['view' => 'active']) }}" class="tab-item {{ $view === 'active' ? 'active' : '' }}">
        <i class="fas fa-user-check"></i>
        Active Users
    </a>
    <a href="{{ route('admin.manage-users', ['view' => 'inactive']) }}" class="tab-item {{ $view === 'inactive' ? 'active' : '' }}">
        <i class="fas fa-user-times"></i>
        Inactive Users
    </a>
    <a href="{{ route('admin.manage-users', ['view' => 'all']) }}" class="tab-item {{ $view === 'all' ? 'active' : '' }}">
...
<div class="filter-bar-card">
    <form method="GET" action="{{ route('admin.manage-users') }}" class="filter-form-flex">
        ...
        <div class="search-wrapper">
            ...
            <input type="text" class="search-input" name="search" placeholder="Search by name, username, email..." value="{{ $filters['search'] }}">
        </div>
        ...
        <select class="filter-select" name="filter_role">
            ...
            <option value="head_hr" {{ $filters['role'] === 'head_hr' ? 'selected' : '' }}>Head HR</option>
            <option value="immediate_head" {{ $filters['role'] === 'immediate_head' ? 'selected' : '' }}>Immediate Head</option>
        </select>
        
        <select class="filter-select" name="filter_office">
            <option value="">All Offices</option>
            @foreach($offices as $office)
                <option value="{{ $office->name }}" {{ $filters['office'] === $office->name ? 'selected' : '' }}>{{ $office->name }}</option>
            @endforeach
        </select>
            ...
            <div class="user-actions">
                ...
                @if($u->id !== auth()->id())
                    <form action="{{ route('admin.users.toggle-status', $u) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PATCH')
                        <button type="submit" class="action-btn action-btn-toggle" title="{{ $u->is_active ? 'Deactivate' : 'Activate' }}">
                            <i class="fas fa-{{ $u->is_active ? 'ban' : 'check' }}"></i>
                        </button>
                    </form>
                    
                    <form action="{{ route('admin.users.delete', $u) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this user?')">
                        @csrf
                        @method('DELETE')
