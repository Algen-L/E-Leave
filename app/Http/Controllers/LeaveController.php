    ...
    public function generateForm6($id)
    {
        ...
        try {
            ...
            if (request('format') === 'pdf') {
                ...
                $tempDocx = $tempDir . '/Leave_' . $application->id . '_' . time() . '.docx';
                $outputPdf = $tempDir . '/Leave_' . $application->id . '_' . time() . '.pdf';
                ...
                try {
                    ...
                    $scriptPath = base_path('scripts/convert-to-pdf.ps1');
                    $cmd = "powershell -ExecutionPolicy Bypass -File \"{$scriptPath}\" \"{$tempDocx}\" \"{$outputPdf}\"";
